...
// The Instructor/Trainer's Google Calendar, pay via Razorpay (INR), then book.
...
export default function CalendarBooking({ consultation, coupons = [], flashSale }) {
  const { user } = useAuth();
  ...
  // Verified coupon preview for the chosen session (null = no discount).
  ...
  const handleChange = (e) => {
    // Changing the session invalidates any applied coupon — it was priced and
    // scope-checked against the previous one. (CouponField also resets itself
    // on remount; clearing here covers deselecting the session entirely, when
    // the field unmounts and cannot report back.)
    if (e.target.name === "sessionId") setCouponQuote(null);
    setForm({ ...form, [e.target.name]: e.target.value });
  };

  const selectedSession = consultation?.sessions?.find((s) => String(s.id) === String(form.sessionId));
  const listPrice = priceToNumber(selectedSession?.price);
  // Display hint only — /api/create-order recomputes this from the session id.
  ...
  const finalizeBooking = async (payResult = {}) => {
    const res = await api.bookSlot({ start: selectedSlot.start, end: selectedSlot.end, ...form });
    const savedNote = payResult.discount > 0 ? ` You saved ${formatRupees(payResult.discount)}.` : "";
    setStatus({
      type: "success",
      message: "✅ Payment received & slot booked! " + res.message + savedNote,
  ...
  const handleBook = async (e) => {
    ...
    try {
      ...
      if (payResult.success) {
        ...
        await finalizeBooking(payResult);
    ...
    <div id="booking-form" className="mt-14 scroll-mt-24">
      <div className="card !p-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h3 className="font-display text-xl font-bold">📅 Book & Pay for a Slot on the Instructor/Trainer's Calendar</h3>
        ...
        <form onSubmit={handleBook} className="mt-8 grid gap-4 border-t border-slate-100 pt-6 sm:grid-cols-2">
          ...
          <input className="input-field" name="email" type="email" inputMode="email" autoComplete="email" placeholder="Your email * (invite is sent here)" value={form.email} onChange={handleChange} required />
          <input className="input-field" name="phone" type="tel" inputMode="tel" autoComplete="tel" placeholder="Phone / WhatsApp" value={form.phone} onChange={handleChange} />
          ...
          <textarea className="input-field sm:col-span-2" name="notes" rows="3" placeholder="Anything the Instructor/Trainer should know? (optional)" value={form.notes} onChange={handleChange} />

          {/* A coupon needs a session to price against, so this only appears
              once the visitor has picked one. */}
          {selectedSession && (
          ...
          <button type="submit" disabled={submitting || !selectedSlot} className="btn btn-primary sm:col-span-2 disabled:opacity-60">
            {submitting
              ...
              ? `Pay ${couponQuote ? formatRupees(couponQuote.finalAmount) : selectedSession.price} & Book`
