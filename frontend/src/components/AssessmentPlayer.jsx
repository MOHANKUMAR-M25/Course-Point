...
//             is made, then the Instructor/Trainer reviews and scores it
//
...
export default function AssessmentPlayer({ courseId, courseTitle, onClose, onFinished }) {
        ...
        {needsReview ? (
          <>
            ...
            <p className="mt-2 text-sm leading-relaxed text-slate-600">
              The Instructor/Trainer will listen to each answer and score your fluency, pronunciation and content. You'll get an
  ...
  const requestQuestions = async () => {
    ...
    try {
      ...
      setRequestNote(res.message || "Request sent! The Instructor/Trainer has been notified.");
    } catch (err) {
    ...
    if (!status?.exists || noQuestions) {
      return (
        <div className="rounded-2xl bg-white p-6 shadow ring-1 ring-slate-100 sm:p-8">
          ...
          <p className="mt-2 text-sm leading-relaxed text-slate-600">
            The assessment for <b>{courseTitle}</b> isn't ready — the Instructor/Trainer hasn't published any questions for it yet.
          ...
          {requestNote ? (
            ...
            >
              {requesting ? "Sending request…" : "🔔 Request questions from the Instructor/Trainer"}
            </button>
          )}
