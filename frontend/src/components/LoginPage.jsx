...
export default function LoginPage({ onContinueAsStudent }) {
  ...
  const [form, setForm] = useState({ name: "", email: "", phone: "", identifier: "" });
  ...
  const handleLogin = async (e) => {
    ...
    setError("");
    try {
      const res = await api.student.login({ email: form.identifier });
  ...
  const handleVerifyLogin = async (e) => {
    ...
    setError("");
    try {
      const res = await api.student.verifyLogin({ email: loginEmail, otp: loginOtp });
      loginStudent(res.token, res.student);
      onContinueAsStudent?.();
    } catch (err) {
      setError(err.message);
      ...
      <div className="relative w-full max-w-md animate-fade-in-up">
        ...
        <div className="rounded-3xl bg-white/95 p-8 shadow-2xl ring-1 ring-white/20 backdrop-blur sm:p-10">
          ...
          <div className="text-center">
            <span className="brand-mark mx-auto h-14 w-14 text-lg">CP</span>
            <h1 className="mt-4 font-display text-2xl font-extrabold text-slate-900">
              Welcome!
            </h1>
            <p className="mt-1 text-sm text-slate-500">
              Learn with <span className="font-semibold text-german-red">Course Point</span>
            </p>
          </div>
          ...
          <div className="mt-8">
            ...
            {role === "student" && (
                ...
                {studentMode === "login" && step === "form" && (
                  <form onSubmit={handleLogin} className="mt-4 space-y-3">
                    <input
                      type="email"
                      className="w-full rounded-xl border-2 border-slate-200 px-4 py-2.5 text-sm outline-none transition focus:border-german-red"
                      placeholder="Registered email or Candidate ID *"
                    ...
                    <p className="text-center text-xs text-slate-400">
                      Use your email or CID. We'll email a one-time code to your registered email.
                    </p>
                  </form>
                )}
              ...
              {/* Google sign-in is available only to existing users. */}
              {studentMode === "login" && <div className="mt-4 border-t border-slate-100 pt-4">
