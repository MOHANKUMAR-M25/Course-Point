...
function Stat({ label, value, color = "text-slate-900", icon }) {
  return (
    <div className="min-w-0 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
      <div className="flex items-center justify-between">
        <p className="min-w-0 break-words pr-2 text-xs font-semibold uppercase tracking-wide text-slate-400">{label}</p>
...
function Table({ columns, rows, empty, search = "" }) {
  const normalizedSearch = search.trim().toLowerCase();
  const visibleRows = normalizedSearch
    ? (rows || []).filter((row) => Object.values(row).join(" ").toLowerCase().includes(normalizedSearch))
    : rows;
  if (!visibleRows?.length) return <p className="p-6 text-center text-sm text-slate-400">{normalizedSearch ? "No matching records." : empty}</p>;
  return (
...
export default function Dashboard() {
  ...
  return (
    <section id="dashboard" className="min-h-screen bg-slate-900 py-28 sm:py-32">
      <div className="container-site">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button onClick={load} className="btn btn-gold w-full !py-2 text-sm sm:w-auto">🔄 Refresh</button>
        </div>
          ...
          <>
            ...
            {tab !== "Overview" && (
              <div className="mt-4 max-w-md">
                <label htmlFor="dashboard-search" className="sr-only">Search dashboard records</label>
                <input
                  id="dashboard-search"
                  className="input-field w-full !bg-slate-50 !py-3 text-sm"
                  type="search"
