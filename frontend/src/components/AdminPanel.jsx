...
import GradingAdmin from "./GradingAdmin";
...
export default function AdminPanel({ gallery = [], trainer, testimonials = [], courses = [], onContentChanged }) {
  ...
  const courseToPayload = (c) => ({
    level: c.level || "NEW",
    trainerName: c.trainerName?.trim() || "",
    title: c.title,
  ...
  const visibleCourses = normalizedCourseSearch
    ? courses.filter((c) =>
        [c.level, c.title, c.description, c.duration, c.mode, ...(c.features || [])]
          .join(" ")
          .toLowerCase()
        ...
        <div className="mt-6 rounded-2xl bg-slate-800 p-6">
          ...
          {tab === "Course" && (
            ...
            >
              ...
              <input className="input-field" placeholder="Trainer name for certificate *" value={course.trainerName} onChange={(e) => setCourse({ ...course, trainerName: e.target.value })} required />
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <h3 className="font-display text-sm font-bold uppercase tracking-wider text-german-gold">
                  Courses ({visibleCourses.length}{visibleCourses.length !== courses.length ? ` of ${courses.length}` : ""})
                </h3>
                <label className="relative min-w-[min(100%,20rem)] flex-1 sm:flex-none">
                  <span className="sr-only">Search courses in admin</span>
                  <input
                ...
                <div key={c.id} className="rounded-xl bg-slate-900/60 p-4">
                  {editingCourse?.id === c.id ? (
                    <div className="grid gap-3 sm:grid-cols-2">
                      ...
                      <input className="input-field !py-2 text-xs" placeholder="Trainer name for certificate" value={editingCourse.trainerName || ""} onChange={(e) => setEditingCourse({ ...editingCourse, trainerName: e.target.value })} />
              ...
              {!visibleCourses.length && <p className="py-6 text-center text-sm text-slate-400">No matching courses.</p>}
            </div>
          )}

          {/* Course modules — students tick these to unlock the assessment */}
          {tab === "Modules" && <ModulesAdmin courses={courses} />}

          {/* Assessment builder (written auto-scored / oral trainer-scored) */}
          {tab === "Assessments" && <AssessmentsAdmin courses={courses} />}

          {/* Oral answers awaiting review */}
          {tab === "Grading" && <GradingAdmin />}

          {/* Coupons & flash sale banner */}
          {tab === "Coupons & Sale" && (
            <CouponsAdmin courses={courses} onContentChanged={onContentChanged} />
          )}
