...
export default function Navbar() {
  ...
  const profileName = user?.name || (isGuest ? "Guest" : "Student");
  const profileLabel = user?.cid ? `${profileName} (${user.cid})` : profileName;
    ...
    <header className="fixed inset-x-0 top-0 z-50">
      ...
      >
        <div className="container-site flex h-16 items-center gap-4">
          {/* Compact brand lockup keeps navigation clear on small screens. */}
          <a href="#home" className="flex min-w-0 shrink items-center gap-2" aria-label="Course Point home">
            <span className="brand-mark" aria-hidden="true">CP</span>
            <span className="truncate whitespace-nowrap font-display text-base font-bold text-german-red sm:text-lg">
              Course Point
            </span>
          </a>
