...
export default function Courses({ courses, coupons = [], flashSale, onContentChanged }) {
  ...
  const visibleCourses = normalizedSearch
    ? courses.filter((course) =>
        [course.level, course.title, course.description, course.duration, course.mode, ...(course.features || [])]
          .join(" ")
          .toLowerCase()
  ...
  const saveEdit = async () => {
    ...
    try {
      await api.admin.updateCourse(editing.id, {
        ...
        trainerName: editing.trainerName?.trim() || "",
        title: editing.title,
  ...
  return (
    <section id="courses" className="min-h-screen bg-slate-50 py-28">
      <div className="container-site">
        <h2 className="section-title">
          Explore <span>Courses</span>
        </h2>
        <p className="section-subtitle">
          Find the right learning program and enroll instantly with secure payment (UPI, GPay, PhonePe, cards & net banking).
        </p>

        <div className="relative mt-8 max-w-2xl">
          <label htmlFor="course-search" className="sr-only">Search courses</label>
          <svg className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" aria-hidden="true">
          ...
          <input
            ...
            type="search"
            placeholder="Search by course name, level, topic or format"
            value={search}
            ...
            <div key={course.id} id={`course-${course.id}`} className="card relative flex flex-col scroll-mt-28">
              ...
              {isAdmin && editing?.id === course.id ? (
                <div className="space-y-2">
                  ...
                  <input className="input-field !py-2 text-xs" placeholder="Trainer name for certificate" value={editing.trainerName} onChange={(e) => setEditing({ ...editing, trainerName: e.target.value })} />
                ...
                <>
                  ...
                  <div className="mt-4 space-y-1 text-xs text-slate-500">
                    <p>⏱ {course.duration}</p>
                    <p>💻 {course.mode}</p>
                    <p>👤 {course.trainerName || "Instructor/Trainer"}</p>
                  </div>
        ...
        {!visibleCourses.length && (
          <p className="mt-12 rounded-2xl bg-white p-8 text-center text-sm text-slate-500 shadow-sm ring-1 ring-slate-100">
