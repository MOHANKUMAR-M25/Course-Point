 ...
 *
 * Pass `courseId` (a course) or `sessionId` (a consultation session) so the
 * BACKEND looks up the real price and re-validates `couponCode` itself. The
 * `amount` below is only a display hint / legacy fallback — whatever the
 * server returns in `order.amount` is what actually gets charged.
 *
 * @returns Promise resolving to { success, message } or throwing on error
 */
export async function payWithRazorpay({
  amount, // rupees — display hint only when courseId/sessionId is given
  name,
  email,
  phone,
  sessionName,
  ...
  // 1) Create the order — the server decides the final amount.
  ...
  // The verified amount actually charged (rupees), for receipts & emails.
  const chargedAmount = order.payableAmount ?? amount;
  ...
  return new Promise((resolve) => {
    ...
    const rzp = new window.Razorpay({
      ...
      name: "Course Point — Instructor/Trainer",
