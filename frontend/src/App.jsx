...
// Reads "#/course/<id>" from the URL hash (null = not on a course page).
...
const SALE_DISMISSED_KEY = "gt_sale_dismissed";
...
function Site() {
  ...
  // Owned here (not inside the banner) because the Hero's top padding depends
  // on whether the sale strip is taking up space below the fixed navbar.
  const [saleDismissed, setSaleDismissed] = useState(
    () => sessionStorage.getItem(SALE_DISMISSED_KEY) === "1"
  );
  // Hash routing for the dedicated course page (#/course/<id>)
  const [courseRouteId, setCourseRouteId] = useState(courseIdFromHash);
  const [showCourses, setShowCourses] = useState(() => window.location.hash === "#courses");
  const [showDashboard, setShowDashboard] = useState(() => window.location.hash === "#dashboard");

  useEffect(() => {
    const onHashChange = () => {
      setCourseRouteId(courseIdFromHash());
      setShowCourses(window.location.hash === "#courses");
  ...
  const dismissSale = () => {
    sessionStorage.setItem(SALE_DISMISSED_KEY, "1");
  ...
  // `/api/site` returns the sanitized flash sale + publicly advertisable
  // coupons (hidden referral codes are stripped server-side).
  const sale = site.flashSale?.active && !saleDismissed ? site.flashSale : null;
  const coupons = site.coupons || [];

  // Dedicated course page — replaces the whole home page (hash route).
  if (courseRouteId != null) {
    return (
      <>
        <Navbar hasSpokenGerman={Boolean(site.spokenGerman?.enabled)} />
        <CoursePage
          courseId={courseRouteId}
          course={site.courses?.find((c) => Number(c.id) === courseRouteId)}
          onBack={() => {
  ...
  return (
    <>
      <Navbar hasSpokenGerman={Boolean(site.spokenGerman?.enabled)} />
      <main>
        <FlashSaleBanner sale={sale} onDismiss={dismissSale} />
        <Hero trainer={site.trainer} onContentChanged={loadSite} tightTop={Boolean(sale)} />
        ...
        <AdminPanel
          ...
          onContentChanged={loadSite}
        />
        {/* Renders only for signed-in students — their courses, module
            checklists and assessments. */}
        <MyLearning />
        <About trainer={site.trainer} />
        <Consultation consultation={site.consultation} coupons={coupons} flashSale={site.flashSale} />
