...
// Uses the configured admin's OAuth tokens to:
...
export async function getAvailableSlots() {
  ...
  if (!calendar) {
    // Calendar not connected — show standard working-hours availability
    // until the trainer signs in with Google to link her live calendar.
    return {
      connected: false,
      note: isGoogleConfigured()
        ? "The trainer has not connected her Google Calendar in this session yet. Showing standard working-hours availability — bookings are still recorded and confirmed by email in real time."
        : "Google credentials not configured. Add them to backend/.env to enable live calendar sync.",
...
export async function bookSlot({ start, end, studentName, studentEmail, studentPhone, sessionName, notes }) {
  ...
  if (!calendar) {
    // Calendar not linked in this session — booking is still recorded and
    // real-time confirmation emails are sent to the student and admins.
    return {
      calendarLinked: false,
      eventLink: null,
      message:
        "Booked! A confirmation email has been sent to you. " +
        "(The trainer's Google Calendar will sync this booking once she signs in with Google.)"
    };
  }
