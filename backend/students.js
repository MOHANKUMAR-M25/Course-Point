...
import { findStudentByCid, upsertStudent } from "./db.js";
...
export async function verifySignup({ email, emailOtp, phoneOtp }) {
  email = (email || "").trim().toLowerCase();
  ...
  // Persist to Supabase and assign a unique Candidate ID (CID)
  try {
    ...
    if (data?.cid) student.cid = data.cid;
  } catch (err) {
    console.warn("⚠️ Could not persist student / assign CID:", err.message);
  }
  ...
  sendNotificationEmail({
    ...
    html: `
      <div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;padding:24px;border:1px solid #eee;border-radius:12px">
        ...
        <p style="margin-top:14px;color:#888;font-size:12px">Welcome aboard! — Course Point, Instructor/Trainer</p>
      </div>`
...
export async function startLogin({ email, cid }) {
  const identifier = String(email || cid || "").trim();
  if (!identifier) throw httpError(400, "Enter your registered email address or Candidate ID.");

  let student;
  let loginEmail;
  if (identifier.includes("@")) {
    loginEmail = identifier.toLowerCase();
    if (!isValidEmail(loginEmail)) throw httpError(400, "Please provide a valid email address.");
    student = students.get(loginEmail);
  } else {
    const normalizedCid = identifier.toUpperCase();
    student = [...students.values()].find((candidate) => candidate.cid?.toUpperCase() === normalizedCid);
    if (!student) {
      const record = await findStudentByCid(normalizedCid);
      if (record) {
        student = { name: record.name || "", email: record.email, phone: record.phone || "", cid: record.cid };
        students.set(record.email, student);
      }
    }
  ...
  await sendEmailOtp(loginEmail, otp);
  pendingLogins.set(loginEmail, { otp, expiresAt: Date.now() + OTP_TTL_MS, attempts: 0 });

  return { email: loginEmail, message: `Login code sent to the registered email for this account.` };
}
