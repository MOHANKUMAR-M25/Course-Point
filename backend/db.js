...
export async function findStudentByCid(cid) {
  if (!supabase || !cid) return null;
  const { data, error } = await supabase
    .from("students")
    .select("email, name, phone, cid")
    .eq("cid", String(cid).trim().toUpperCase())
    .maybeSingle();
  if (error) {
    console.error("⚠️ findStudentByCid failed:", error.message);
    return null;
  }
  return data || null;
}
...
export async function findStudentByEmail(email) {
  ...
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) {
    console.error("⚠️ findStudentByEmail failed:", error.message);
    return null;
  }
  return data || null;
}
