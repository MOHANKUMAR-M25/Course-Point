...
import { startSignup, verifySignup, startLogin, verifyLogin, students } from "./students.js";
...
import { isDbConfigured, upsertStudent, findStudentByEmail, saveContactSubmission } from "./db.js";
import {
  getPublicFlashSale,
  getPublicCoupons,
  resolveOrderContext,
...
app.get("/api/auth/google/callback", async (req, res) => {
  try {
    ...
    if (isAdmin) {
      ...
      existingStudent = students.get(email) || await findStudentByEmail(email);
      if (!existingStudent) {
        return res.redirect(
          `${FRONTEND_URL}/?auth_error=${encodeURIComponent("New students must register with email and phone OTP first.")}`
        );
      }
    }
    ...
    const sessionToken = issueSessionToken({
      email,
      name: existingStudent?.name || profile.name,
      picture: profile.picture,
      phone: existingStudent?.phone || "",
      cid: existingStudent?.cid || null,
      isAdmin
    });
...
app.post("/api/contact", async (req, res) => {
  ...
  res.status(201).json({
    success: true,
    message: `Thank you, ${name}! Your message has been received. The Instructor/Trainer will reply to ${email} within 24 hours.`
  });
});
    ...
    sendNotificationEmail({
      ...
      html: `
        <div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;padding:24px;border:1px solid #eee;border-radius:12px">
          ...
          <p style="margin-top:14px;color:#888;font-size:12px">See you there! — Course Point, Instructor/Trainer</p>
        </div>`
...
app.post("/api/admin/courses", requireAdmin, (req, res) => {
  ...
  if (!trainerName) return res.status(400).json({ error: "Trainer name is required for the course certificate." });
  const course = { id: Date.now(), ...req.body, trainerName };
...
app.put("/api/admin/courses/:id", requireAdmin, (req, res) => {
  ...
  if (patch.trainerName !== undefined) {
    patch.trainerName = String(patch.trainerName || "").trim();
    if (!patch.trainerName) return res.status(400).json({ error: "Trainer name is required for the course certificate." });
  }
...
// on every boot and never overwrites the Instructor/Trainer's edits.
