...
/**
 * Any signed-in account with an email — used by the learning and assessment
 * routes, which are keyed on the student's email. Guests (who never get a
 * token) and unauthenticated visitors are rejected.
 *
 * Note this only proves identity. Whether that student may actually open a
 * course or assessment is decided by the enrollment + progress checks in
 * learning.js / assessments.js.
 */
export function requireStudent(req, res, next) {
  if (!req.user?.email) {
    return res.status(401).json({
      error: "Please sign in (or create a student account) to track your learning."
    });
  }
  next();
}
