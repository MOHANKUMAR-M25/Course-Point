  ...
  sendNotificationEmail({
    ...
    html: `
      <div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;padding:24px;border:1px solid #eee;border-radius:12px">
        ...
        <p style="margin-top:14px;color:#888;font-size:12px">Your consultation is confirmed. — Course Point, Instructor/Trainer</p>
      </div>`
  }).catch((e) => console.error("⚠️ success email failed:", e.message));

  sendSms(phone, `Payment of ${INR(amount)} for ${sessionName} was successful. Your consultation is confirmed! — Course Point, Instructor/Trainer`)
  ...
  sendNotificationEmail({
    ...
    html: `
      <div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;padding:24px;border:1px solid #eee;border-radius:12px">
        ...
        <p style="margin-top:8px;color:#888;font-size:12px">— Course Point, Instructor/Trainer</p>
      </div>`
  }).catch((e) => console.error("⚠️ failed email failed:", e.message));

  sendSms(phone, `Your payment of ${INR(amount)} for ${sessionName} failed. No money deducted. Please try again. — Course Point, Instructor/Trainer`)
